...
'''
    ...
    if start_idx != -1 and end_idx != -1:
        # We want to keep the column structure, so we replace from start_tag but keep the ending col-lg-8 div close
        # Actually, let's just replace the INNER content of col-lg-8
...
'''
    ...
    if ss_idx != -1 and se_idx != -1:
        content = content[:ss_idx] + new_sidebar_html + content[se_idx:]
...
for filename, data in services_data.items():
    page_html = build_page(filename, data)
    with open(os.path.join(directory, filename), 'w', encoding='utf-8') as f:
        f.write(page_html)

print("Absolute fix applied to all service pages.")
