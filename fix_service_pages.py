...
def generate_page(filename, data):
    ...
    # 1. Update Head Title and Header Title
    ...
    # 2. Update Main Content Heading and Desc
    content = content.replace("<h2>Explore our Service Lists</h2>", f"<h2>{data['title']} Solutions</h2>")
    p_pattern = r'<p>Our business consulting services are designed to help organizations navigate complexity.*?</p>'
    content = re.sub(p_pattern, f'<p>{data["desc"]}</p>', content, flags=re.DOTALL)
    
    # 3. Update Image 1
    content = content.replace('./assets/images/services/service-details-img-1.jpg', data["main_img"])
    
    # 4. Update Features List 1 (Insights & Strategies)
    features_html = "".join([f'<li><i class="pxni-checked"></i> {f}</li>' for f in data["features"]])
    # Match the first list after Professional Insights
    list_pattern = r'<h3>Professional Insights & Strategies</h3>\s*<ul class="pxn_list">.*?</ul>'
    content = re.sub(list_pattern, f'<h3>Professional Insights & Strategies</h3>\n                      <ul class="pxn_list">\n                        {features_html}\n                      </ul>', content, flags=re.DOTALL)
    
    # 5. Update Documents Required
    docs_html = ""
    for title, desc in data["docs"]:
        docs_html += f'''
    ...
    docs_pattern = r'<h2>Document Required</h2>\s*<div class="icon_box_wrap mb-30">.*?</div>'
    content = re.sub(docs_pattern, f'<h2>Document Required</h2>\n                  <div class="icon_box_wrap mb-30">{docs_html}\n                  </div>', content, flags=re.DOTALL)
    
    # 6. Update Image 2 (Video Section)
    content = content.replace('./assets/images/services/service-details-img-2.jpg', data["second_img"])
    
    # 7. Update Sidebar Active Link and Titles
    # Generate sidebar links
    sidebar_links_html = ""
    for f, d in services_data.items():
        active_class = "active" if f == filename else ""
        sidebar_links_html += f'''
                      <li>
                        <a class="service {active_class}" href="{f}">
    ...
    sidebar_pattern = r'<ul class="pxn_services_list">.*?</ul>'
    content = re.sub(sidebar_pattern, f'<ul class="pxn_services_list">{sidebar_links_html}\n                    </ul>', content, flags=re.DOTALL)
    
    # 8. Cleanup "Read More" site-wide inside this page
...
for filename, data in services_data.items():
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(generate_page(filename, data))

print("Service pages re-generated correctly.")
